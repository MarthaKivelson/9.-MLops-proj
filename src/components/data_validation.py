...
class DataValidation:
    ...
    def is_column_exist(self, df: DataFrame) -> bool:
        ...
        try:
            ...
            missing_numerical_colums = []
            for column in self._schema_config["numerical_columns"]:
                 if column not in dataframe_columns:
                      missing_numerical_colums.append(column)
            
            if len(missing_numerical_colums) > 0:
                 logging.info(f"Missing numerical column: {missing_numerical_colums}")
            ...
            return False if len(missing_categorical_columns)>0 or len(missing_numerical_colums)>0 else True
    ...
    def initialize_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                ...
                logging.info(f"All categorical/num features are present in the train dataset: {status}")

            status = self.is_column_exist(df=test_df)
            if not status:
                validation_error_msg += f"Columns are missing in test dataframe"
            else:
                logging.info(f"All categorical/num features are present in the test dataset: {status}")
            ...
            #ensure dir for validation exist
